...
export default function ProductsShowCase() {
    return (
        <div className="w-full">
            <h1 className="text-2xl font-bold capitalize mb-2">Unique Gifts for Every Occasion</h1>
            <p className="text-md font-medium text-muted-foreground">Discover thoughtful and creative gift ideas for birthdays, anniversaries, holidays, and more.</p>
            ...
            <div className="w-full h-full ">
                <div className="p-4 border relative">
                    <div className="absolute inset-0 h-full w-full pointer-events-none z-10" >
                        <Plus strokeWidth={1} className="absolute -top-4 -left-4 size-8 text-muted-foreground" />
                        <Plus strokeWidth={1} className="absolute -top-4 -right-4 size-8 text-muted-foreground" />
                        <Plus strokeWidth={1} className="absolute -bottom-4 -left-4 size-8 text-muted-foreground" />
                        <Plus strokeWidth={1} className="absolute -bottom-4 -right-4 size-8 text-muted-foreground" />
                    </div>
                    <div className="rounded-sm overflow-hidden transition-transform duration-300 ease-in-out">
                        <img draggable={false} className="select-none transition-transform duration-300 ease-in-out hover:scale-105 w-full h-auto object-cover aspect-square" src={product.images[0]} alt={product.title} width={500} height={500} />
                    </div>
            ...
            {Array.from({ length: 5 }).map((_, i) => {
                ...
                return (
                    <div key={i} className="relative w-4 h-4">
                        ...
                        <Star
                            className="absolute top-0 left-0 size-4 text-yellow-500 fill-yellow-500"
                            style={{ clipPath: `inset(0 ${100 - fillPercentage}% 0 0)` }}
                        />
                    </div>
                );
            })}
            <span className="text-sm text-muted-foreground ml-1">{ratings}</span>
        </div>
    )
}